...
f32 = Frame(window3, width=350, height=400, bg="#ffb16e")
f32.place(x=30, y=140)

nameL3 = Label(f32, text='Full name ', padx=5, pady=5, font=('Arial', 12, "bold"), bg="#ffb16e", fg="white")
nameL3.place(x=10, y=10)

nameE3 = Entry(f32, textvariable=name3_var, width=25)
nameE3.place(x=150, y=14)

regL3 = Label(f32, text='Registration no.', padx=5, pady=5, font=('Arial', 12, "bold"), bg="#ffb16e", fg="white")
regL3.place(x=10, y=60)

regE3 = Entry(f32, textvariable=reg3_var, width=25)
regE3.place(x=150, y=64)

comL = Label(f32, text='Company Name', padx=5, pady=5, font=('Arial', 12, "bold"), bg="#ffb16e", fg="white")
comL.place(x=10, y=110)

comE = Entry(f32, textvariable=com3_var, width=25)
comE.place(x=150, y=114)

cgpaL3 = Label(f32, text='CGPA', padx=5, pady=5, font=('Arial', 12, "bold"), bg="#ffb16e", fg="white")
cgpaL3.place(x=10, y=160)

cgpaE3 = Entry(f32, textvariable=cgpa3_var, width=25)
cgpaE3.place(x=150, y=164)

sectorL = Label(f32, text='Sector', padx=5, pady=5, font=('Arial', 12, "bold"), bg="#ffb16e", fg="white")
sectorL.place(x=10, y=210)

sectorE = Entry(f32, textvariable=sector3_var, width=25)
sectorE.place(x=150, y=214)

yearL3 = Label(f32, text='Year Of Joining', padx=5, pady=5, font=('Arial', 12, "bold"), bg="#ffb16e", fg="white")
...
def submit3():
    conn = sqlite3.connect('registration_student.db')
    cur = conn.cursor()
    cur.execute("INSERT INTO selected_student VALUES (?, ?, ?, ?, ?, ?)",
...
def update3():
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE selected_student SET Full_name = ?, Company_name = ?, CGPA = ?, Sector = ?, Year_of_joining = ? WHERE Reg_no = ?",
...
def delete3():
    conn = sqlite3.connect('registration_student.db')
    cur = conn.cursor()
    cur.execute("DELETE FROM selected_student WHERE Reg_no=?", (reg3_var.get(),))
    conn.commit()
    conn.close()
    DisplayData3()
    messagebox.showinfo("Record deleted", f"{reg3_var.get()} this record is deleted successfully")
...
def search3():
    conn = sqlite3.connect('registration_student.db')
    cur = conn.cursor()
    cur.execute("SELECT * FROM selected_student WHERE Reg_no=?", (search3_var.get(),))
    result = cur.fetchone()
    if result:
        ...
        messagebox.showinfo("Error", "No record found with the given registration number")
    conn.close()
...
upd_b3 = Button(text='View', padx=5, pady=5, command=View)
upd_b3.place(x=180, y=600)

frame13 = Frame(window3, width=840, height=450, bg="#ffb16e")
frame13.place(x=400, y=140)

search_frame3 = Frame(frame13, height=100, width=500, bg="#ffb16e")
...
# Create the "selected" table if it doesn't exist
cur.execute('''CREATE TABLE IF NOT EXISTS selected_student (
               Full_name TEXT,
...
# For example, the DisplayData3() function can be defined here and called later in your script
def DisplayData3():
    conn = sqlite3.connect('registration_student.db')
    cur = conn.cursor()
    cur.execute("SELECT * FROM selected_student")
